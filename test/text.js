...
// db.items.find({$eq:[ISODate("$expire_date").getMonth(),ISODate().getMonth()+1]})

// db.items.aggregate([{$match:{"expire_date":{$gte:ISODate().getMonth()+1}}}])
// db.items.find({$where:function(){

//   return (new Date(this.expire_date)).getMonth()+1<= (new Date()).getMonth();
  
// }})

// //
// db.items.aggregate([{$project:{month: {$month: '$expire_date'},itemname:1}},{$match:{month:(ISODate().getMonth()+1)}}])

// db.items.aggregate([{$project:{month: {$month: '$expire_date'},itemname:1}},{$match:{month:{lte:(ISODate().getMonth()+2)}}},{$project:{itemname:1,_id:0}}])

// db.items.aggregate([{$project:{expire_limit:{$dateAdd:{startDate:ISODate(),unit:"month",amount:1}},itemname:1}},{$match:{expire_limit:{$lt:"$expire_month"}}}])
